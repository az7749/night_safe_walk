...
| 기능 | 상태 | 현재 구현/남은 작업 |
| --- | --- | --- |
| 안전 인프라 지도 보기 | `△ 진행 중` | 현재 워킹트리 기준 `MapScreen`은 현재 위치 이동, 지도 탭 기반 경로 지점 선택, 검색 결과 카메라 이동, bounds 기준 도로 오버레이 재조회, 반경 30m 내 신고 후보 시설물 표시, 지도 범위 내 접수된 고장 신고 마커 표시까지 연결돼 있다. 다만 상시 시설물 레이어 범례와 `final_score` 기준 안내는 아직 없다. |
...
| 기능 | 상태 | 현재 구현/남은 작업 |
| --- | --- | --- |
| 안전 시설물 신고 | `○ 완료` | 지도에서 현재 위치 기준 반경 30m 내 신고 후보 시설물을 불러오고, 시설물 종류/거리 확인 후 사진 촬영, 고장 유형 선택, 설명 입력을 거쳐 `/reports`로 실제 신고를 접수할 수 있다. |
| 신고 내역 조회 | `○ 완료` | 더보기 시트에서 `ReportHistoryScreen`으로 이동해 `/users/{userId}/reports` 기반 신고 내역, 사진, 접수 시각, 처리 상태를 조회할 수 있다. |

### 1.6 관리자

| 기능 | 상태 | 현재 구현/남은 작업 |
| --- | --- | --- |
| 관리자 신고 검수 | `△ 진행 중` | 로그인 응답의 `role`이 `admin`이면 `AdminReportScreen`으로 분기하고, 검수 대기/승인됨/처리 이력 탭에서 신고 목록을 조회한 뒤 상세 화면에서 승인, 반려, 처리 완료를 `/admin/reports` API로 갱신할 수 있다. 다만 관리자 권한 정책, 운영자 계정 흐름, 실서버 검증은 아직 남아 있다. |
...
| 진행 중 | 비밀번호 재설정/프로필 수정 화면과 API 연동, bounds 기반 도로 오버레이 조회, 장소 검색 패널과 빠른길/안전한길 버튼을 포함한 경로탐색 시제품, 위험 도로 진입 알림 시제품, 관리자 신고 검수 흐름 |
| 보류/미구현 | 시설물 상시 레이어/범례 정리, 경로 품질 검증, SOS, 알림 내역/설정, 관리자 권한 검증, 비상 연락처, 즐겨찾기 관리 |
...
- 시설물 신고와 관리자 검수 흐름은 앱 화면과 API 호출이 연결된 만큼 권한 정책, 상태 전이 규칙, 이미지 업로드 실패 케이스를 실서버 기준으로 검증한다.
...
- 확인: 지난 실행 시각인 2026-07-30 09:47 KST 이후 새 커밋은 없고, 이번 갱신 대상은 워킹트리에 누적된 시설물 신고 작성/조회와 관리자 검수 기능이다.
- 완료: `lib/features/report/screen/facility_report_screen.dart`, `lib/features/report/service/facility_report_service.dart`, `android/app/src/main/AndroidManifest.xml`, `ios/Runner/Info.plist`, `pubspec.yaml` 기준으로 사진 촬영, 고장 유형 선택, 설명 입력, `/reports` 멀티파트 업로드까지 연결된 실제 시설물 신고 접수 흐름이 추가됐다.
- 완료: `lib/features/report/screen/report_history_screen.dart`, `lib/features/main/main_screen.dart`, `lib/components/bottom_sheets/more_bottom_sheet.dart` 기준으로 사용자 신고 내역 화면 진입과 `/users/{userId}/reports` 기반 상태 조회가 연결됐다.
- 진행 중: `lib/features/admin/screen/admin_report_screen.dart`, `lib/features/admin/service/admin_report_service.dart`, `lib/features/auth/screen/login_screen.dart` 기준으로 관리자 로그인 분기, 검수 대기/승인됨/처리 이력 조회, 승인/반려/처리 완료 액션이 추가됐지만 운영 권한 정책과 실서버 검증 기록은 아직 없다.
- 진행 중: `lib/features/map/map_screen.dart`는 지도 bounds 기준 접수된 신고 마커 재조회와 상세 바텀시트까지 렌더링하지만, 상시 시설물 범례와 `final_score` 기반 안전도 기준 통일은 여전히 남아 있다.
- 보류: 저장소와 자동화 메모리, 현재 문서에서 다음 세미나 전 해야 할 일에 대한 새 사용자 전달 사항은 추가로 발견되지 않아 기존 "다음 세미나까지 할 일" 목록을 중복 없이 유지한다.
