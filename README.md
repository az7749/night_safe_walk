...
| 장소 검색·즐겨찾기 | 장소 검색, 출발지·목적지 설정 및 즐겨찾기 관리 |
| 경로 선택 | 빠른길·안전한길 버튼으로 원하는 경로 요청 |
| 시설물 신고 | 주변 가로등·보안등 신고, 사진 첨부 및 처리 내역 확인 |
| 위험구역 알림 | 위험구역 진입 감지, 알림 설정 및 내역 관리 |
| 비상 연락처·SOS | 비상 연락처 관리 및 현재 위치를 포함한 SOS 요청 |
| 회원 기능 | 로그인·회원가입, 프로필 수정 및 비밀번호 재설정 |
| 관리자 기능 | 신고 처리, 회원·시설물 조회 및 수정 |

앱은 로그인 화면으로 시작하며, 로그인한 계정의 역할에 따라 사용자 지도 또는 관리자 화면으로 이동합니다. 백그라운드 위험구역 감지와 SOS 수신 알림은 Android에서 동작하도록 구현되어 있습니다.
...
| 사진 첨부 | `image_picker` |
| 백그라운드·알림 | `flutter_background_service`, `flutter_local_notifications` |
...
경로 API 주소는 [route_api_service.dart](lib/features/map/service/route_api_service.dart), 도로 API 주소는 [road_overlay_service.dart](lib/features/map/service/road_overlay_service.dart)에 있습니다. 다른 API를 사용할 경우 해당 서비스의 주소도 확인해야 합니다. 백엔드 코드는 [safe_walk_server](https://github.com/az7749/safe_walk_server)에서 관리합니다.
...
| `GET /route` | 출발지와 도착지 사이 경로 조회 | `s_lat`, `s_lng`, `e_lat`, `e_lng`, `mode` (`fast` / `safe`) |
...
4. 길 안내 패널에서 빠른길 또는 안전한길을 눌러 지도에 표시된 경로를 확인합니다.
...
└── features/
    ├── auth/                       # 인증 관련 화면 및 로직
    ├── main/
    │   └── main_screen.dart        # 메인 화면 및 기능 연결
    └── map/
